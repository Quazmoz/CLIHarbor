...
Probe execution is direct, shell-free, non-interactive, temporary-working-directory based, time/output bounded, and supplied only a minimal environment. Raw captured bytes are sanitized before display/export. Exit code, timeout, cancellation, truncation, stdout, and stderr remain evidence; non-zero exit is never rewritten as success.

The typed export schema is `cliharbor.phase0/v1`. It contains build/host identity, sanitized tool discovery state, declared probe identity, and bounded sanitized captures. It intentionally contains no resolved executable/candidate path, raw environment, browser/session tokens, credential-store contents, or arbitrary user files.
