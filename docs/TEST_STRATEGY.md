...
- Windows CI downloads the exact pinned CyberArk Conjur v9.3.1 amd64 release asset, independently rechecks its reviewed size/SHA-256, qualifies it through CLIHarbor `doctor --tool-path`, and runs the declared root help probe through the same isolated probe boundary used on managed laptops;
