...
| Run the real Conjur 9.x pack | Load `packs/conjur/conjur-v9.yaml` explicitly after normal CLIHarbor qualification |
...
### 1. Download the qualified artifact
...
Do not substitute a copied standalone EXE. The executable, Phase 0 pack and `EVALUATION_SHA256SUMS` form one qualified bundle.
...
Do not run directly from inside the ZIP and do not add the real Conjur pack to this directory before preflight.
...
After preflight succeeds, keep the real pack **outside** the immutable evaluation bundle, for example:

```text
%USERPROFILE%\Downloads\CLIHarbor-Packs\conjur-v9.yaml
```

Then inspect readiness:

```bat
bin\cliharbor-windows-x64-evaluation.exe doctor --pack-file "%USERPROFILE%\Downloads\CLIHarbor-Packs\conjur-v9.yaml"
```

If the discovered `conjur.exe` is compatible, start the browser UI:

```bat
bin\cliharbor-windows-x64-evaluation.exe serve --pack-file "%USERPROFILE%\Downloads\CLIHarbor-Packs\conjur-v9.yaml"
```

The trusted Conjur pack requires CLI version `>=9.3.1 <10.0.0` and exposes only reviewed read-only, non-secret workflows. It uses the vendor CLI's existing session (`vendor-session`); CLIHarbor does not ask for or persist Conjur passwords, tokens or MFA values.

If you are not already authenticated, use your organization's approved Conjur/vendor login process, then retry the read-only workflow.

See [Conjur CLI 9.x Integration](docs/CONJUR_INTEGRATION.md) for the exact command surface and provenance.
...
```bat
bin\cliharbor-windows-x64-evaluation.exe doctor --pack-file "%USERPROFILE%\Downloads\CLIHarbor-Packs\conjur-v9.yaml" --tool-path "cyberark-conjur-v9/conjur=C:\Program Files\Approved Tool\conjur.exe"
```

Use the same reviewed `--tool-path` value when starting `serve`.

CLIHarbor fails closed on ambiguous discovery rather than picking the first PATH match.
...
| Conjur command reports signed out | Authenticate through the approved vendor-owned flow; CLIHarbor does not collect credentials |
